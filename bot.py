...
# ✅ Обмеження
MAX_LESSONS_PER_DAY = 2  # Максимум 2 години на день
...
from database import (
    ...
    update_lesson,
    add_lesson_rating,
    check_student_has_booking_at_time,  # ✅ ДОДАНО
    count_student_bookings_on_date,     # ✅ ДОДАНО
    can_block_time_slot                 # ✅ ДОДАНО
)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_instructor:
            ...
            text = "👋 Привіт, інструктор!\n\nОберіть дію:"
            
            if is_admin(user_id):
                keyboard.append(["🔐 Панель адміна"])
...
async def handle_schedule_management(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "block_choose_date":
        ...
        if not can_block_time_slot(instructor_id, date_formatted, time_start, time_end):
            await update.message.reply_text(
