# database_imports.py
# Helper functions to import instructors, students and lessons into the SQLite DB
...
def import_students(students: List[Dict[str, Any]]) -> bool:
    """Import students. Handles fields like price_per_hour and tariff."""
    try:
        with get_db() as conn:
            cursor = conn.cursor()
            for s in students:
...
def import_lessons(lessons: List[Dict[str, Any]]) -> bool:
    """
    Import lessons. Resolves instructor_id by name if missing and student's telegram id by looking up students.
    """
    try:
        with get_db() as conn:
            cursor = conn.cursor()
