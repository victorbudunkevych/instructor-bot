...
# Telegram handler that parses an uploaded Excel file and calls database import helpers.
...
# mapping helper to normalize headers seen in exported files
...
async def import_from_excel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # is_admin check - bot.py defines is_admin; caller should wire this handler and the check
    try:
        from bot import is_admin
    except Exception:
        # best-effort: if is_admin not available, deny to be safe
        await update.message.reply_text("❌ Admin check not available in this context.")
        return
    ...
    try:
        ...
        if instructors:
            ...
            # clear tables even if no instructor sheet
        ...
        report = []
        report.append(f"Instructors: {len(instructors)} - {'OK' if ok_ins else 'FAILED'}")
        report.append(f"Students: {len(students)} - {'OK' if ok_students else 'FAILED'}")
        report.append(f"Lessons: {len(lessons)} - {'OK' if ok_lessons else 'FAILED'}")

        await update.message.reply_text("\n".join(report))

    except Exception as e:
        logger.error(f"Error in import_from_excel handler: {e}", exc_info=True)
